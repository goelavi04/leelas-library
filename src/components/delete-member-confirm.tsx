...
import type { DeleteMemberState } from "@/app/admin/members/actions";
...
function ConfirmButton() {
  ...
  return (
    ...
    >
      {pending ? "Deleting…" : "Yes, delete this member"}
    </button>
  );
}
...
export function DeleteMemberConfirm({
  ...
  memberName: string;
  action: (prevState: DeleteMemberState, formData: FormData) => Promise<DeleteMemberState>;
}) {
  const [state, formAction] = useActionState<DeleteMemberState, FormData>(action, {});

  return (
    <div className="max-w-lg rounded-xl border border-line shadow-card bg-paper p-8">
      <h1 className="text-xl font-bold tracking-tight text-ink">Delete this member?</h1>
      <p className="mt-3 text-[15px] text-ink-soft">
        Are you sure you want to delete <strong className="text-ink">{memberName}</strong>? This cannot be
        undone.
      </p>
